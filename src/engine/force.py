"""
force.py — Capacidad de fuerza, fatiga e incidentes (§4.2).
...
def evaluar_riesgo(
    ...
    *,
    ...
    SOBRE LA SATURACIÓN
    -------------------
    El producto crudo no está acotado: militares fatigados, de noche, en un nodo
    ...
    riesgo_mitigado = riesgo * factor
    p = min(P.P_INCIDENTE_MAX, 1.0 - math.exp(-riesgo_mitigado))
...
def ejecutar_operacion(
    ...
    *,
    ...
    """
    Aplica fuerza sobre un nodo. Resuelve el incidente con la semilla del motor.

    El azar es estocástico pero reproducible: la corrida entera se puede repetir
    en el debriefing con una decisión cambiada.
    """
    ev = evaluar_riesgo(
    ...
    # El desgaste de la operación sobre las unidades
    for u in unidades:
    ...
    # El éxito de la apertura no depende del incidente: se puede abrir el nodo
    # y producir una catástrofe reputacional al mismo tiempo.
    prob_exito = max(0.15, 1.0 - nodo.dureza * 0.6)
    exito = rng.random() < prob_exito

    msg = f"Operación sobre {nodo.nombre}: "
    msg += "nodo despejado. " if exito else "no se logró despejar. "
    if hubo_incidente:
    ...
    for u in estado.unidades:
        if u.asignacion == "relevo":
            ...
            if u.fatiga <= 0.05:
                ...
                u.ubicacion = None
        elif u.asignacion in ("contencion", "operacion", "escolta", "custodia"):
...
def capacidad_inmovilizada_por_custodia(estado: Estado) -> int:
    ...
    """
    n = len(estado.instalaciones_criticas)
    return n * P.CUSTODIA_POLICIAS_POR_INSTALACION


def solicitar_relevo(estado: Estado, n_unidades: int) -> int:
    """
    Intercambio explícito: menor probabilidad de catástrofe reputacional a
    cambio de menor cobertura simultánea.
    """
    candidatas = sorted(
