...
//       Policía, y en el tablero dejaría sin oficio a uno de los nueve.
// ---------------------------------------------------------------------------
...
import {
  COLOR_CORREDOR, INTERVENCION, INTERVENCION_CORTA, rotulo,
} from '../etiquetas.jsx'
...
const HECHO = {
  ...
  punto_verificado: { rango: 0, color: '#7099cf', texto: 'lo verificó una dupla' },
}
...
export default function Mapa({ tablero, seleccionado, onSeleccionar }) {
  ...
  // La ficha de abajo: lo fijado con un clic manda sobre lo que se está mirando.
  // En una pantalla proyectada, quien mueve el ratón no es quien habla.
  const fijado = seleccionado ? { tipo: 'punto', id: seleccionado } : null
  const foco = fijado || sobre || (zoom ? { tipo: 'region', id: zoom } : null)
...
{/* DEBAJO del punto final, no encima. El nombre de un punto va arriba
          ...
          {sitios.filter(s => s.tipo !== 'ciudad').map(s => (
            <g key={s.id} style={{ pointerEvents: 'none' }}>
              {s.tipo === 'puerto' ? (
                <>
                  <circle cx={s.x} cy={s.y} r={2.6 * esc} fill="#7fa3d8"
                          fillOpacity={0.12} />
                  <path d={`M ${s.x - 1.5 * esc} ${s.y + 0.5 * esc} h ${3 * esc}
                            M ${s.x} ${s.y - 1.6 * esc} v ${2.9 * esc}`}
                        stroke="#8fb6e4" strokeWidth={0.5 * esc} fill="none"
                        strokeLinecap="round" />
                  <circle cx={s.x} cy={s.y - 2 * esc} r={0.65 * esc}
                          fill="none" stroke="#8fb6e4" strokeWidth={0.45 * esc} />
                </>
              ) : (
                <rect x={s.x - 0.9 * esc} y={s.y - 0.9 * esc}
                      width={1.8 * esc} height={1.8 * esc}
              ...
              <text x={s.x} y={s.y - 4.4 * esc} textAnchor="middle"
          ...
          {zoom && infraestructura.filter(i => i.region_id === zoom).map(i => (
            <g key={i.infra_id} style={{ pointerEvents: 'none' }}
              ...
              <circle cx={i.x} cy={i.y} r={2.5 * esc}
              ...
              <path d={INFRA[i.tipo] || INFRA.logistica}
                    transform={`translate(${i.x} ${i.y}) scale(${0.8 * esc})`}
                    fill="none" stroke={i.protegida ? '#a8d8e8' : '#e0b5a4'}
                    strokeWidth={0.55} strokeLinecap="round"
                    strokeLinejoin="round" />
              <text x={i.x} y={i.y + 4.6 * esc} textAnchor="middle"
                    fontSize={1.85 * esc}
          ...
          {puntos.map(p => {
            ...
            const dx = (ancla === 'end' ? 2.6 : ancla === 'start' ? -2.6 : 0) * esc
            const dy = -((lineas.length > 1 ? 5.9 : 3.8) + (piso[p.nodo_id] || 0) * PISO)
...
{/* El anillo de lo que pasó anoche: un halo tenue por debajo y el
                ...
                {sinMirar && (
                  <text x={p.x} y={p.y + 1.0 * esc} textAnchor="middle"
                        fontSize={2.7 * esc} fill="#a7b1c2" fontWeight={600}
      ...
      <Ficha foco={foco} fijado={Boolean(fijado)} puntos={porId}
             regiones={regiones} corredores={corredores}
             onSoltar={() => onSeleccionar?.(null)} />
...
// ---------------------------------------------------------------------------
// LA FICHA — las seis lecturas, en palabras
//
// Un punto y una región dicen exactamente las mismas seis cosas, y eso es
// deliberado: la sala aprende a leer una ficha, no dos. Lo único que cambia es
// que la región añade su recuento —cuántos puntos, cuántos cerrados— y el punto
// añade a qué corredor pertenece.
//
...
function decir(clave, m, esRegion) {
  ...
  if (clave === 'dias_sostenido') {
    if (!esRegion) return `${m.dias} ${m.dias === 1 ? 'día' : 'días'} · ${m.banda}`
    return `${m.dias} de media · el más antiguo, ${m.dias_max}`
  }
  return m.banda
}

export function Ficha({ foco, fijado, puntos, regiones, corredores, onSoltar }) {
  if (!foco) {
    return (
      <p className="ficha-vacia">
        Pase por encima de una región para ver sus cifras, o toque para acercarse.
      </p>
    )
  }

  const esRegion = foco.tipo === 'region'
  const cosa = esRegion ? regiones[foco.id] : puntos[foco.id]
  const lectura = esRegion ? cosa?.lectura : cosa?.lectura
  if (!cosa || !lectura || lectura.sin_puntos_modelados) {
    return (
      <p className="ficha-vacia">
        {cosa?.nombre || '—'}: este ejercicio no modela ningún cierre aquí.
      </p>
    )
  }

  const corredor = !esRegion && corredores.find(c => c.corredor_id === cosa.corredor_id)
  const bloqueo = esRegion ? lectura.bloqueo : null
  const region = !esRegion ? regiones[cosa.region_id] : null

  return (
    <div className={`ficha${fijado ? ' fijada' : ''}`}>
      <div className="ficha-cabeza">
        <div>
          <span className="ficha-nombre">{cosa.nombre}</span>
          {!esRegion && <span className="punto-id">{cosa.nodo_id}</span>}
          {esRegion && cosa.epicentro && <span className="eyebrow"> · epicentro</span>}
        </div>
        {bloqueo && (
        ...
        {fijado && (
          <button className="ficha-soltar" onClick={onSoltar}>Soltar</button>
        )}
      </div>
      ...
      <div className="ficha-sub">
        {esRegion
          ...
          : [region?.nombre,
             ...
             cosa.verificado_turno ? `constatado en la jornada ${cosa.verificado_turno}`
               : 'nadie lo ha verificado',
            ].filter(Boolean).join(' · ')}
        {!esRegion && (
          <>
            {' · '}
            <span className={`chip chip-${cosa.intervencion === 'fuerza' ? 'mal'
              : cosa.intervencion === 'negociacion' ? 'bien' : 'neutro'}`}>
              {rotulo(INTERVENCION, cosa.intervencion)}
            </span>
            {/* UNA MESA INSTALADA Y UNA MESA INSTALADA HOY no son la misma
                cosa: la primera existe, la segunda avanza. Sin esta línea, una
                sala instala una mesa la jornada 1 y da por hecho que sigue
                trabajando sola hasta la 5, que es exactamente lo que no pasa. */}
            {cosa.mesa && (
              <span className={`chip chip-${cosa.mesa.sesionada_hoy ? 'bien' : 'medio'}`}>
                {cosa.mesa.sesionada_hoy
                  ? 'mesa instalada hoy'
                  : cosa.mesa.jornadas_congelada
                    ? `mesa congelada · ${cosa.mesa.jornadas_congelada} jornada`
                      + `${cosa.mesa.jornadas_congelada === 1 ? '' : 's'}`
                    : 'mesa sin sesionar hoy'}
              </span>
            )}
          </>
        )}
        {/* QUÉ SE ESTÁ HACIENDO EN ESTA REGIÓN, contado. El promedio de caudal
            no lo dice: cuatro puntos cerrados sobre los que nadie hace nada y
            cuatro con mesa instalada dan el mismo promedio, y son dos regiones
            distintas. */}
        {esRegion && lectura.intervencion && (
        ...
        {esRegion && cosa.semaforo && (
          <>
            {' · abastecimiento '}
            <span className={`chip chip-${cosa.semaforo === 'verde' ? 'bien'
              : cosa.semaforo === 'ambar' ? 'medio' : 'mal'}`}>
              {cosa.semaforo}
            </span>
          </>
        )}
          ...
          return (
            <div key={clave} className="ficha-fila">
              ...
              <dd style={{ color: tinta(m) }}>
                ...
                {/* Solo en un punto. En una región, `constatado` es «los
                    constató a todos», y decir «sin constatar» de una región con
                    cuatro de cinco verificados sería afirmar algo falso. La
                    constatación es un hecho de un punto. */}
...
/**
 * Solo la clave de formas y de anillos. Los nombres de los corredores los ponen
 * el propio mapa y la tabla de al lado: tres leyendas de lo mismo eran dos de más.
 */
function Formas() {
  return (
    <>
      <div className="mapa-formas">
        <span className="eyebrow" style={{ letterSpacing: '0.08em' }}>Qué se hace</span>
        <span>◆ A la fuerza</span>
        <span>■ En negociación</span>
        <span>● Nada</span>
        <span>? Sin verificar</span>
        <span><i className="aro" style={{ borderColor: '#7fc4a8' }} /> Mesa hoy</span>
        <span><i className="aro punteado" style={{ borderColor: '#c9a05a' }} /> Mesa congelada</span>
        <Ayuda etiqueta="Qué significa cada forma">{D.formas_mapa}</Ayuda>
      </div>
      <div className="mapa-formas">
        <span className="eyebrow" style={{ letterSpacing: '0.08em' }}>Última ventana</span>
